  ...
  useEffect(() => {
    ...
    return () => {
      if (restoreFocus && previouslyFocusedElement.current) {
        const elementToFocus = typeof restoreFocus === 'function' 
          ? restoreFocus() 
...
export function useKeyboardNavigation(props: KeyboardNavigationProps = {}) {
  ...
  const handleKeyDown = useCallback((event: React.KeyboardEvent) => {
    switch (event.key) {
      ...
      case 'ArrowUp':
      case 'ArrowDown':
        if (arrowNavigation === 'vertical' || arrowNavigation === 'both' || arrowNavigation === 'grid') {
          event.preventDefault();
      ...
      case 'ArrowLeft':
      case 'ArrowRight':
        if (arrowNavigation === 'horizontal' || arrowNavigation === 'both' || arrowNavigation === 'grid') {
          event.preventDefault();
...
export function useAnnouncements(props: AnnouncementProps = {}) {
  ...
  const announceText = useCallback((text: string, priority: 'polite' | 'assertive' = announcementLevel) => {
  ...
  const announceChange = useCallback((value: any) => {
    if (announceOnChange) {
