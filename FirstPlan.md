...
Prompt: "Initialize a Bun project and configure the package.json for a React + TypeScript + Storybook setup. Run bun init and modify package.json to include React, TypeScript, Storybook dependencies (@storybook/react, @storybook/addon-essentials, @storybook/addon-docs). Create bunfig.toml with Bun configuration for module resolution and package management. Add .nvmrc file with Node 20 for fallback compatibility. Create tsconfig.json with strict TypeScript configuration for components and website."
